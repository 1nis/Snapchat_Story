...
class SnapchatDownloader2025:
    ...
    def _configure_session(self):
        ...
        try:
            ...
            self.logger.critical(
                "Échec configuration session: %s", str(e), exc_info=True
            )
            raise
    ...
    def _get_story_data(self, username: str) -> dict:
        ...
        try:
            ...
            self.logger.error(
                "Erreur HTTP %d: %s", e.response.status_code, str(e), exc_info=True
            )
            return {}
    ...
    def download_stories(self, username: str):
        ...
        try:
            ...
            if not media_data:
                self.logger.warning(
                    "Liste de médias vide - Vérifier : %s",
                    self._get_profile_url(username),
                )
                return

            save_dir = f"{username}_{datetime.now().strftime('%Y%m%d')}"
            ...
            for idx, (url, media_type) in enumerate(media_data, 1):
                try:
                    ...
                    self.logger.debug(
                        "Téléchargement %d/%d: %s", idx, len(media_data), url
                    )
                    ...
                    self.logger.error(
                        "Échec téléchargement média: %s", str(e), exc_info=True
                    )
